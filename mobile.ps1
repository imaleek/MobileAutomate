...
Function Kill-Process {
    ...
    if(!(Get-Process  -Name $config.AppToKill -ErrorAction SilentlyContinue)){
        $config.Result = 'True'
        }else{
        Kill-Process $Server
        }
...
Function delete-files {
    Param
        ...
        remove-item -Path $filepath1 -Recurse -Force 
        }
        catch{
                $errormsg = $_.ToString()
                $exception = $_.Exception
        ...
        remove-item -Path $filepath2 -Recurse -Force 
        }
        catch{
                $errormsg = $_.ToString()
                $exception = $_.Exception
...
Function service-start {
    Param
        ...
        if((Get-service -Name $config.Service -ComputerName $servers).Status -eq 'Running'){
